...
namespace samplebrowser.Controllers
{
    public partial class TreeGridController : Controller
    {
        ...
        public ActionResult TreeGridExport()
        {
            ViewBag.datasource = this.GetExportEditingDataSource();
            ViewBag.toolbarItems = new List<String>(){"pdfExport","excelExport"};
            return View();
        }

        private List<BusinessObject> GetExportEditingDataSource()
        {
        ...
        public ActionResult ExportToExcel(string TreeGridModel)
        {
            ExcelExport exp = new ExcelExport();
            var DataSource = GetExportEditingDataSource();
            TreeGridProperties treeGridProp = ConvertGridObject(TreeGridModel);
            TreeGridExcelExport excelExp = new TreeGridExcelExport();
            excelExp.FileName = "Export.xlsx";
            excelExp.ExportSettings = new TreeGridExportSettings() { Theme = ExportTheme.FlatSaffron };            
            excelExp.ExcelVersion = ExcelVersion.Excel2010;            
            return exp.Export(treeGridProp, DataSource, excelExp);
        }

        public ActionResult ExportToPdf(string TreeGridModel)
        {
            PdfExport exp = new PdfExport();
            var DataSource = GetExportEditingDataSource();
            TreeGridProperties treeGridProp = ConvertGridObject(TreeGridModel);
            TreeGridPdfExport pdfExp = new TreeGridPdfExport();
            pdfExp.FileName = "Export";
            pdfExp.ExportSettings = new TreeGridExportSettings() { Theme = ExportTheme.FlatSaffron };
            return exp.Export(treeGridProp, DataSource, pdfExp);
        }
        private TreeGridProperties ConvertGridObject(string treeGridProperty)
        {
            ...
            treeGridProp = (TreeGridProperties)JsonConvert.DeserializeObject(treeGridProperty, typeof(TreeGridProperties));
            return treeGridProp;
        }
